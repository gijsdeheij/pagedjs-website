...
Sorry for the silence, November came and with it, all the slowness of winter, which, this year, got the form of a quite strong covid. The good thing is that it’s away now, and I’m back up and running, and it’s the perfect moment to announce one of the great wins for paged.js rebuilding session we’re hard working for. 
...
I got the luxury of getting into cssPrint using book.js, a project founded by Adam Hyde (like paged.js) that was using CSS Regions to generate page. You made a change on page 2, and all the book was rebuilding from there. And we could quickly try things using the inspector and everything was visible on screen. But CSS Regions were dropped by the Chromium team and what was amazing just disappeared. So we didn’t update Chrome for quite a while. And WebKit followed, and everything about CSS regions disappeared. 

I ended up using a very precise Chromium under a very special Ubuntu version on a virtual machine for a long time (and we didn’t have SSD then, everything was *so* long). I wasn’t the only one struggling, the folks at Open Source Publishing ended up making [their own fork of webkit to support their design work](blog.osp.kitchen/residency/from-webkit-to-ospkit.html). We, designers, love to experiment with our tools, and the browser is one of the most accessible setup for that. We need to make a poster? We’re gonna build our own boilerplate for previewing on screen how our HTML and CSS will render in print. And to do so, we spend quite a long time to figure out the HTML and CSS that will allow that.

But most of the time, this html and this css is literally a simplest version of what paged.js generates. We have a `<div>` representing the page, and will have CSS attached to handle the preview of the screen: setting the size of the `<div>` to be equal to what you’ll set in the `@page`, make sure to set the overflow accordingly, and, if you really want to reuse the system, you’re setting up a precise CSS grid, with margins, space for running headers, etc.

And through the game of copy/pasting, we’re rebuilding the same thing every time we need, and it’s not fun, because we always forget a line, or get into a new bug, because copy and pasting isn't free from errors.
...
The HTML standard defines a set of tags that supported by compativle browsers. We all use these tages from one web page to another. `<h1>` is a level one title, `<p>` is a paragraph and `<figure>` an element that will be understood by readers and computers, as a figure. Those tags come with attributes (like `id` or `class` for the most well known ones), and a behavior that has been defined by the W3C, and implemented in a similar way in all the browsers. A good example? an `<img>` tag has a `alt` attribute. This attribute gives an opportunity to the author to add a description of the image that will be picked up by a screen reader and read aloud to, for example, a person with visibility issues. This behavior is described in the specs, and provides guidance on how an element will work.

A web-component is essentially a custom tag which extends the default set of HTML-tags. The Web Components API's allow authors to define their own tags with javascript, HTML and CSS.

A web-component will include HTML to define its structure (the template), CSS (the default styles), and javascript to influence their behavior (how the element will react to a user). To use a web-component in your page, you only need to include the javascript file which defines all those, and then you can simply add the component where you need in your webpage.

One of my favorite web-components is a preview of a keyboard called [x-keyboard](https://onedeadkey.github.io/x-keyboard/) (and its [source](https://github.com/OneDeadKey/x-keyboard)). It’s the perfect tool to show a keyboard layout on the screen, and it will even be able to pick up what the visitor will type to and react accordingly. 
...
```html
<paged-page>
  <h1>The store will be closed for Christmas. See you in January!</h1>
</paged-page>
```

All the CSS for the preview, and for the print, comes with the component. This is already a win. 

And we’ve gone a bit further, and added attributes to handle all the needed CSS directly from the HTML element: bleed, marks, margin, width and height are added as attributes and transfered to the CSS and the HTML.
...
Pretty easy, right? I hope you’re gonna enjoy this, we’re really excited about the possibilities this will offer.
...
Well, you can’t. The specs do not allow to style the content within a `@page`-block. You can influence the page itself. So you have margins, bleeds, marks and it's possible to set the geometry of the page with the size property, but that’s it. 
...
And then, think of all the weird things you could do with text-size: if the text needs to be huge on page 10, making it huge might push it to page 11, what should you have on the page 10 then?
...
But it’s a bit of a lucky game, because sometimes, you don’t want that to happen, and your stuck with this *not-a-bug-a-feature* that paged.js offers. Thankfully, in the world of web-components, it’s a bit different. Picture this HTML:
...
```css
#page-12::part(bottom-center) {
  text-transformation: uppercase;
  font-size: .8em
}

#page-12::part(bottom-right)::after {
...
`<paged-page>` is a component that is waiting for these attributes:
  ...
  - `margin`: the size of the margins of the page, written down exactly as their css counterpart. 
...
If you happen to have multiple pages with multiple page size, saving it as a PDF in Chromium based browsers will create a PDF with different page sizes.


## About margins

Since we were quite happy with the paged-margins, we got into exploring margin-boxes as web-components. As you may know, in the land of CSSPrint, the page has a limited of places where you can add things in the margins. THe specifications says that there are 17 margin-boxes, following this diagram:
...
<paged-page>
    <paged-margins slot="margins">
        ...
        <paged-margin-content slot="left-top">🖨️</paged-margin-content>
    </paged-margins>

    <h2>there are margins  !</h2>
</paged-page>
```

In that HTML, we have a page, that contains the margins, and the rest of the content will fill up the page-area. 

We hope you'll love playing with it, we’re excited to include it in paged.js!
